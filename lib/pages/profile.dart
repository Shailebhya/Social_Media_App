...
import 'package:flutter14app/pages/edit_profile.dart';
...
import 'package:flutter14app/widgets/post_tile.dart';
...
return Container(
  ...
  buildProfileButton() {
    //we have to find if it is the current user ad if it is then show the edit profile button
...
//make auth user to unfollower of another user
  followersRef
      .document(widget.profileId)
...
// delete acivity feed item for themadd activity feed item to notify them that we are following them
  activityFeedRef
      .document(widget.profileId)
...
followingRef
  ...
  .document(widget.profileId)
  .setData({});
// add activity feed item to notify them that we are following them
activityFeedRef
  .document(widget.profileId)
  .collection('feedItems')
  .document(currentUserId)
